---
title: "IWG-Alf Parts for Grain Paper 2"
author: "Leah Treffer"
date: "2026-01-14"
...
In excel, the following plots were removed from Yr 3 MN because they were reported as weak alfalfa regrowth or completly overrun with weeds. Since the lack of competition from alfalfa would influence IWG growth, thier removal from analysis may reduce outlier effect. Deleting these plots resulted in deleting 27 instances (summer, fall, spring)
...
```{r}
data<-read.csv('data/Original_Data/MasterDataSpreadsheet2.csv')
```
...
scale_x_continuous(breaks = c(0, 40, 80, 120, 160))+ #fix x axis scale
  ...
  facet_wrap(~P, scales = "free_y",ncol=2)+ #make it by location (remove to make it across locations)
  scale_color_discrete(name = "Cropping System")  #legend for colors
# save figures/grain2/grain_Nrate_yr3
...
sorted_df <- new_df %>%
  arrange(P)
#view(sorted_df)
# write.csv(sorted_df, "data/Intermediate_Data/Nrate_esteff_grain_yr3_2.csv")
```
...
# gt_NG |> gtsave("figures/grain2/gt_NG.png", expand = 10)
```
...
GraincomboplotNE <- ggplot(data=NdXSIG) +
  ...
  geom_function(data=subset(NdZSI, P== "MN"), fun=~36.50*exp(0.0133*.x),color='orange')+
...
# ggsave("figures/grain2/GraincomboplotNE.jpg", plot = GraincomboplotNE, width = 8, height = 6, dpi = 300)
```
...
print(GXPC)
# ggsave("figures/grain2/GXPC.jpg", plot = GXPC, width = 8, height = 6, dpi = 300)
```
...
GZPC<-ggplot(NdZSI, aes(x = P, y = exp(predicted), color = CROP)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
    scale_y_continuous(breaks = c(250, 500, 750, 1000, 1250), limits = c(0, 1260))
print(GZPC)
# ggsave("figures/grain2/GZPC.jpg", plot = GZPC, width = 8, height = 6, dpi = 300)
```

# combined grain years plots E:C
```{r}
GEC<-grid.arrange(GXPC, GYPC, GZPC, ncol = 2)
# ggsave("figures/grain2/GEC.jpg", plot = GEC, width =12, height = 6, dpi = 350)
```
...
print(GLERE)
# ggsave("figures/grain2/GPLER_yr3_E.jpg", plot = GLERE, width = 8, height = 6, dpi = 300)
```
...
# ggsave("figures/grain2/GPLERcomboplot.jpg", plot = GPLERcomboplot, width = 8, height = 6, dpi = 300)
```
...
GRYZcomboplot<-ggplot(NGRY_longZ, aes(x = N, y = GRelativeYield)) +
  ...
  ggtitle("Year 3 Grain Relative Yield")+
    coord_cartesian(ylim = c(NA, NA))
# ggsave("figures/grain2/GRYZcomboplot.jpg", plot = GRYZcomboplot, width = 8, height = 6, dpi = 300)
...
GRYZcomboplot2<-ggplot(NGRY_longZ, aes(x = N, y = GRelativeYield)) +
  ...
  ggtitle("Year 3 Grain Relative Yield")+
    coord_cartesian(ylim = c(NA, NA))+
  geom_text(data = merged_df, aes(x = N, y = response, label = .group), color = "black", vjust = -6)
# ggsave("figures/grain2/GRYZcomboplot2.jpg", plot = GRYZcomboplot2, width = 8, height = 6, dpi = 300)
rm(cld_df,merged_df,emmsNP)
...
# ggsave("figures/grain2/GRYcomboplot.jpg", plot = GRYcomboplot , width = 8, height = 6, dpi = 300)
```
