...
knitr::kable(modNXST1_coeff, caption = "Y1 Summer Mixed-Effects Model: log(PlotTotForageKgHa) ~ Nrate * Location * CROPsystem + (1 | Rep)") %>%
  kable_styling(full_width = TRUE, position = "center", bootstrap_options = c("striped", "hover", "condensed")) %>% readr::write_file("figures/table_significant_coeff_yr1S.html")
...
colnames(modNXFT1_coeff)[5]<-"Pr(>t)"
 #Create custom table using kable and kableExtra
knitr::kable(modNXFT1_coeff, caption = "Y1 Fall Mixed-Effects Model: log(PlotTotForageKgHa) ~ Nrate * Location * CROPsystem + (1 | Rep)") %>%
  kable_styling(full_width = TRUE, position = "center", bootstrap_options = c("striped", "hover", "condensed")) %>% readr::write_file("figures/table_significant_coeff_yr1F.html")
...
# Create custom table using kable and kableExtra
knitr::kable(modNYST_coeff, caption = "Summer 2023 Mixed-Effects Model: PlotTotForageKgHa ~ Nrate * Location * CROPsystem + (1 | Rep)") %>%
  kable_styling(full_width = TRUE, position = "center", bootstrap_options = c("striped", "hover", "condensed")) %>% readr::write_file("figures/table_significant_coeff_yr2S.html")
...
# Create custom table using kable and kableExtra
knitr::kable(modNYFT1_coeff, caption = "Yr 2 F Mixed-Effects Model: PlotTotForageKgHa ~ Nrate * Location * CROPsystem + (1 | Rep)") %>%
  kable_styling(full_width = TRUE, position = "center", bootstrap_options = c("striped", "hover", "condensed")) %>% readr::write_file("figures/table_significant_coeff_yr2F.html")
...
# Leah Stopped HERE - Relationship between Total Forage Yield and N-rate looks weird *****
```{r}
NdZS_2 <- NdZS[NdZS$PlotTotForageKgHa != 0, ] # remove rows where the total forage was 0

# WI yr 3 is looking way higher than the previous two years. I am wondering is the data they sent, in kg biomass in quadrat, we was off by a decimal. This would make it more similar to the other two years. 
NdZS_2_WI <- NdZS_2[NdZS_2$Location == 'WI', ]
NdZS_2_WI$PlotIWGDryKG <- NdZS_2_WI$PlotIWGDryKG * 0.1
NdZS_2_WI$PlotIWGDryKgHa <- NdZS_2_WI$PlotIWGDryKG/NdZS_2_WI$QAreaHa
NdZS_2_WI$PlotAlfDryKG <- NdZS_2_WI$PlotAlfDryKG * 0.1
NdZS_2_WI$PlotAlfDryKgHa <- NdZS_2_WI$PlotAlfDryKG/NdZS_2_WI$QAreaHa
NdZS_2_WI$PlotTotForageKgHa <- NdZS_2_WI$PlotIWGDryKgHa + NdZS_2_WI$PlotAlfDryKgHa

NdZS_3 <- NdZS_2[NdZS_2$Location != 'WI', ]
NdZS_3 <- rbind(NdZS_3, NdZS_2_WI)

NdZS_3$P <- as.factor(NdZS_3$P)
NdZS_3$CROP <- as.factor(NdZS_3$CROP)
NdZS_3$Rep <- as.factor(NdZS_3$Rep)
NdZS_3$N <- as.numeric(NdZS_3$N)

#linear
modNZST<-lmer(PlotTotForageKgHa~N*P*CROP + (1|Rep), data=NdZS_3)
anova(modNZST)
#Check Assumptions
...
plot(modNZST)
#Logtransform
modNZST1<-lmer(log(PlotTotForageKgHa)~N*P*CROP + (1|Rep), data=NdZS_3) 
anova(modNZST1)
summary(modNZST1)
#check assumptions logtransform
...
# Create custom table using kable and kableExtra
knitr::kable(modNZST1_coeff, caption = "Yr 3 S Mixed-Effects Model: PlotTotForageKgHa ~ Nrate * Location * #CROPsystem + (1 | Rep)") %>%
  kable_styling(full_width = TRUE, position = "center", bootstrap_options = c("striped", "hover", "condensed")) %>% readr::write_file("figures/table_significant_coeff_yr3S.html")

#linear plots
NdZS_3$predicted <- predict(modNZST1, newdata = NdZS_3, re.form = NA)
ggplot(NdZS_3, aes(x = N, y = PlotTotForageKgHa, color = CROP, group= CROP)) +
    geom_point() + geom_line(aes(N, exp(predicted)))+ #regression through backtransformed predicted values
  labs(x = "N-Rate (Kg/ha)", y = "Total Forage Yield (Kg/ha)") +  # Label the axes
