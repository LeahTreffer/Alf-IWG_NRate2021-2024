...
# Back transformed from sqrt model to normal 
...
emtrends_result<-emtrends(modGsqrt,~ P+YearCode+CROP, var= "N", infer=TRUE)
...
emmeans_results<-emmeans(modGsqrt, ~P+YearCode+CROP, at=list(N=0))
...
merged_df <- emtrends_df %>%
  ...
  select(P, YearCode, CROP, emmean, N.trend, SE.x, plvl)
view(merged_df)
...
new_df <- merged_df %>%
    mutate(equation = sprintf("=(%.2f+(%.4fN))²", emmean, N.trend), SE.x = sprintf("%.4f", SE.x), percent_change = sprintf("%.2f%%", 100 * (2 * N.trend / emmean)))%>%
