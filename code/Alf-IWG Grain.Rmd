...
emtrends_result<-emtrends(modGsqrt,~ P+YearCode+CROP, var= "N", infer=TRUE) # added type="response" to backtransform confidence levels 
...
emmeans_results<-emmeans(modGsqrt, ~P+YearCode+CROP, at=list(N=0)) # added type="response" to backtransform confidence levels 
...
new_df <- merged_df %>%
    #mutate(equation = sprintf("=(%.2f+(%.4fN))²", emmean, N.trend), SE.x = sprintf("%.4f", SE.x), percent_change = sprintf("%.2f%%", 100 * (2 * N.trend / emmean)))%>% # change emmean to response
