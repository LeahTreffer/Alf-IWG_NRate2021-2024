...
sorted_df <- new_df %>%
  arrange(P)
#view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr1.csv")

# percent change in grain yield per unit increase in nitrogen broken out by location and CROP
emtrends_result<-emtrends(modNXG1,~ P*CROP, var= "N", infer=TRUE)
  emtrends_df <- as.data.frame(emtrends_result)
  desired_order_P <- c("MN", "NY", "KS", "WI")
  emtrends_df$P <- factor(emtrends_df$P, levels = desired_order_P)
emmeans_results<-emmeans(modNXG1, ~P*CROP, at=list(N=0))
emmeans_df <- as.data.frame(emmeans_results)
merged_df <- emtrends_df %>%
...
sorted_df <- new_df %>%
  arrange(P)
#view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr1_PC.csv")
...
emtrends_resultX<-emtrends(modNXG1,~ P, var= "N", infer=TRUE)
...
sortedX
#plot
GraincomboplotNEX <- ggplot(data=NdXSIG) +
  geom_point(data = NdXSIG, aes(x = N, y = G, shape = C)) +
  ...
  geom_function(data=subset(NdXSIG, P== "MN"), fun=~179.38*exp(0.0021*.x),color='darkgrey')+
  geom_function(data=subset(NdXSIG, P== "NY"), fun=~408.24*exp(0.0047*.x),color='darkgrey')+
  geom_function(data=subset(NdXSIG, P== "KS"), fun=~407.18*exp(-0.0002*.x),color='darkgrey')+
  geom_function(data=subset(NdXSIG, P== "WI"), fun=~736.77*exp(-0.0001*.x),color='darkgrey')+
  geom_text(data = sortedX, mapping = aes(x = -Inf, y = -Inf, label = equation), hjust   = -0.1, vjust   = -38,  size=4)+ # add equation to figure
  geom_text(data = sortedX, mapping = aes(x = -Inf, y = -Inf, label = plvl), hjust   = -2, vjust   = -20) # add stars to figure
print(GraincomboplotNEX)
# ggsave("figures/grain/GrainplotNEX.jpg", plot = GraincomboplotNEX, width = 8, height = 6, dpi = 300)

#figure E:C (XS)
NdXSIG$predicted <- predict(modNXG1, newdata = NdXSIG, re.form = NA)
XSGemmsPC <- emmeans(modNXG1, ~ P*CROP, type = "response") # Obtain estimated marginal means (excluding N)
emms_df <- as.data.frame(XSGemmsPC)
cld_dataXSGPC <- cld(XSGemmsPC, Letters=custom_letters)
cld_df <-as.data.frame(cld_dataXSGPC)
...
GXSPC<-ggplot(NdXSIG, aes(x = P, y = exp(predicted), shape = CROP)) +
  geom_jitter(width = 0.2, height = 0, size = 3, alpha = 0.7) +
  labs(x = "Environment", y = "Grain Yield (kg ha-1)") +
  ...
  geom_text(data = merged_df, aes(x = P, y = response.x, label = .group), 
            color = "black", vjust = 0, hjust = 1.75, size=6)+ #2.15
...
print(GXSPC)
#ggsave("figures/Paper/GXSPC.jpg", plot = GXSPC, width = 10, height = 8, dpi = 300)

modNXG1<-lmer(log(G)~N*P*CROP + (1|Rep), data=NdXSIG)
#EQUATION tables JUST N 
emtrends_result<-emtrends(modNXG1,~ 1, var= "N", infer=TRUE)
emtrends_df <- as.data.frame(emtrends_result)
emmeans_results<-emmeans(modNXG1, ~1, at=list(N=0))
emmeans_df <- as.data.frame(emmeans_results)
...
GXSN <- ggplot(NdXSIG, aes(x = N, y = G)) +
  geom_point(color='black') + #geom_line(aes(N, exp(predicted)))+ #regression through backtransformed predicted values
  geom_function(data=NdXSIG, fun=~384.99*exp(0.0016*.x),color='black')+
  labs(x = "N-Rate (Kg/ha)", y = "Grain Yield") +  # Label the axes
  ggtitle("Year 1 Summer Relationship between Grain Yield and N-rate \nbacktransformed for visualization") +  # Add a title
scale_x_continuous(breaks = c(0, 40, 80, 120, 160))+ #fix x axis scale
  ylim(0, 1550) +
  geom_abline(intercept = 1, slope = 0, color = "darkgrey", linetype = "longdash")+
  geom_text(data = new_df, mapping = aes(x = 10, y = 1500, label = equation),  size=4)+ # add equation to figure
  theme(axis.text=element_text(size=14), #change font size of axis text
        axis.title=element_text(size=18)) #change font size of axis titles
GXSN
# save figures/Paper/G_Nrate_N_yr1S  900x795
...
NdXSIG$P <- factor(NdXSIG$P, levels = desired_order)
# finding values for the function for each yr:location  
emtrends_resultX<-emtrends(modNXG1,~ N*P, var= "N", infer=TRUE)
  emtrends_dfX <- as.data.frame(emtrends_resultX)
  emtrends_dfX$plvl <-
    ifelse(emtrends_dfX$p.value == 0.05, ".",
    ifelse(emtrends_dfX$p.value < 0.05  & emtrends_dfX$p.value >= 0.01, "*",
  ...
  emtrends_dfX$P <- factor(emtrends_dfX$P, levels = desired_order_P)
emmeans_resultsX<-emmeans(modNXG1, ~N*P, at=list(N=0))
  emmeans_dfX <- as.data.frame(emmeans_resultsX)
mergedX <- emtrends_dfX %>%
  inner_join(emmeans_dfX, by = c("P")) %>%
...
sortedX <- X_df %>%
  arrange(P)
#plot
GcomboplotNEXS <- ggplot(data=NdXSI) +
  geom_point(data = NdXSIG, aes(x = N, y = G)) +
  labs(x = "Nitrogen Rate (kg ha-1)", y = "Grain Yield (kg ha-1)")+
  scale_x_continuous(breaks = c(0, 40, 80, 120, 160)) +
  ylim(0, 2000) +
  facet_wrap(~P, scales = "free_y", ncol = 2,
  ...
  geom_function(data=subset(NdXSIG, P== "NY"), fun=~408.24*exp(0.0047*.x),color='darkgrey')+
  geom_function(data=subset(NdXSIG, P== "WI"), fun=~736.77*exp(-0.0001*.x),color='darkgrey')+
  geom_text(data = sortedX, mapping = aes(x = 35, y = 1800, label = equation),  size=7)+ # add equation to figure
  geom_text(data = sortedX, mapping = aes(x = 80, y = 1800, label = plvl), size=7)+ # add stars to figure
# add stars to figure
  theme(axis.text=element_text(size=22), #change font size of axis text
        ...
        strip.text.x = element_text(size = 26)) #change font size of facet title
print(GcomboplotNEXS)
# ggsave("figures/Paper/final_figures/GcomboplotNEXS.jpg", plot = GcomboplotNEXS, width = 10, height = 8, dpi = 300)
# ggsave("~/Desktop/Y.jpg", plot = GcomboplotNEXS, width = 13, height = 10.4, dpi = 300)
```
...
sorted_df <- new_df %>%
  arrange(P)
view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr2.csv")

# percent change in grain yield per unit increase in nitrogen broken out by location and CROP
emtrends_result<-emtrends(modNYGadj1,~ P*CROP, var= "N", infer=TRUE)
  emtrends_df <- as.data.frame(emtrends_result)
  desired_order_P <- c("MN", "NY", "KS", "WI")
  emtrends_df$P <- factor(emtrends_df$P, levels = desired_order_P)
emmeans_results<-emmeans(modNYGadj1, ~P*CROP, at=list(N=0))
emmeans_df <- as.data.frame(emmeans_results)
merged_df <- emtrends_df %>%
  inner_join(emmeans_df, by = c("P", "CROP")) %>%
  select(P, CROP, emmean, N.trend, SE.x)
...
sorted_df <- new_df %>%
  arrange(P)
view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr2_PC.csv")
...
# finding values for the function for each yr:location  
emtrends_resultY<-emtrends(modNYGadj1,~ P, var= "N", infer=TRUE)
...
emmeans_resultsY<-emmeans(modNYGadj1, ~P, at=list(N=0))
emmeans_dfY <- as.data.frame(emmeans_resultsY)
mergedY <- emtrends_dfY %>%
  inner_join(emmeans_dfY, by = c("P")) %>%
  select(P, emmean, N.trend, SE.x, plvl)
...
sortedY  
#plot
GraincomboplotNEY <- ggplot(data=NdYSI) +
  geom_point(data = NdYSI, aes(x = N, y = G, shape = C)) +
  labs(x = "N-Rate (Kg ha-1)", y = "Grain Yield", linetype = "Cropping System", shape = "Cropping System", color = "Harvest")+
  scale_x_continuous(breaks = c(0, 40, 80, 120, 160)) +
  ylim(0, 1200) +
  facet_wrap(~P, scales = "free_y", ncol = 2) +
  scale_shape_manual(values = c(4, 25),labels = c("Intercropped","Monoculture IWG"))+
  geom_function(data=subset(NdYSI, P== "MN"), fun=~56.23*exp(0.0021*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "NY"), fun=~241.90*exp(0.0010*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "KS"), fun=~113.83*exp(-0.0005*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "WI"), fun=~307.25*exp(0.0065*.x),color='darkgrey')+
  geom_text(data = sortedX, mapping = aes(x = -Inf, y = -Inf, label = equation), hjust   = -0.1, vjust   = -38,  size=2)+ # add equation to figure
  geom_text(data = sortedY, mapping = aes(x = -Inf, y = -Inf, label = plvl), hjust   = -2, vjust   = -20) # add stars to figure
print(GraincomboplotNEY)
# ggsave("figures/grain/GrainplotNEY.jpg", plot = GraincomboplotNEY, width = 8, height = 6, dpi = 300)
...
NdYSI$P <- factor(NdYSI$P, levels = desired_order)
# finding values for the function for each yr:location  
emtrends_resultY<-emtrends(modNYGadj1,~ N*P*CROP, var= "N", infer=TRUE)
  emtrends_dfY <- as.data.frame(emtrends_resultY)
  emtrends_dfY$plvl <-
    ifelse(emtrends_dfY$p.value == 0.05, ".",
    ifelse(emtrends_dfY$p.value < 0.05  & emtrends_dfY$p.value >= 0.01, "*",
  ...
  emtrends_dfY$P <- factor(emtrends_dfY$P, levels = desired_order_P)
emmeans_resultsY<-emmeans(modNYGadj1, ~N*P*CROP, at=list(N=0))
emmeans_dfY <- as.data.frame(emmeans_resultsY)
...
#plot
GraincomboplotNECY <- ggplot(data=NdYSI) +
  geom_point(data = NdYSI, aes(x = N, y = G, shape = CROP)) +
  geom_function(data=subset(NdYSI, P== "KS" & CROP == "inter"), aes(linetype = CROP), fun=~93.35*exp(-0.0029*.x), color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "KS" & CROP == "monoI"), aes(linetype = CROP), fun=~142.46*exp(-0.0001*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "MN" & CROP == "inter"), aes(linetype = CROP), fun=~38.14*exp(0.0052*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "MN" & CROP == "monoI"), aes(linetype = CROP), fun=~82.90*exp(-0.0009*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "NY" & CROP == "inter"), aes(linetype = CROP), fun=~230.39*exp(0.0012*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "NY" & CROP == "monoI"), aes(linetype = CROP), fun=~253.99*exp(0.0008*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "WI" & CROP == "inter"), aes(linetype = CROP), fun=~378.01*exp(0.0042*.x),color='darkgrey')+
  geom_function(data=subset(NdYSI, P== "WI" & CROP == "monoI"), aes(linetype = CROP), fun=~249.74*exp(0.0089*.x),color='darkgrey')+  

  geom_text(data = sortedY_I, mapping = aes(x = 45, y = 1175, label = equation),  size=3.75)+ # add equation to figure
  geom_point(data = sym_I, aes(x = x_sym, y = y_sym), shape = 4, size = 1.5, inherit.aes = FALSE)+
  geom_text(data = sortedY_M, mapping = aes(x = 45, y = 1100, label = equation),  size=3.75)+ # add equation to figure
  geom_point(data = sym_M, aes(x = x_sym, y = y_sym), shape = 25, size = 1.5, fill = "lightgrey", stroke = 0.6, inherit.aes = FALSE)+
  geom_text(data = sym_I, mapping = aes(x = 90, y = 1175, label = plvl), size= 4)+ # add stars to figure
  geom_text(data = sym_M, mapping = aes(x = 90, y = 1100, label = plvl), size= 4)+ # add stars to figure

scale_linetype_manual(name = "Cropping System",
                      labels = c(inter = "Intercropped", inter = "Intercropped", monoI = "Monoculture IWG", monoI = "Monoculture IWG"),
                      values = c(inter = "dotdash", inter = "dotdash", monoI = "dashed", monoI = "dashed")) +
scale_shape_manual(name = "Cropping System",
                     labels = c(inter = "Intercropped", inter = "Intercropped", monoI = "Monoculture IWG", monoI = "Monoculture IWG"),
                     values = c(4, 25, 4, 25))+
  labs(x = "Nitrogen Rate (kg ha-1)", y = "Grain Yield (kg ha-1)", shape = "Cropping System") +
...
print(GraincomboplotNECY)
# ggsave("figures/Paper/final_figures/GrainplotNECY.jpg", plot = GraincomboplotNECY, width = 10, height = 8, dpi = 300)
```
...
sorted_df <- new_df %>%
  arrange(P)
#view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr3.csv")

# percent change in grain yield per unit increase in nitrogen broken out by location and CROP
emtrends_result<-emtrends(modNZG1,~ P*CROP, var= "N", infer=TRUE)
  emtrends_df <- as.data.frame(emtrends_result)
  desired_order_P <- c("MN", "NY", "KS", "WI")
  emtrends_df$P <- factor(emtrends_df$P, levels = desired_order_P)
emmeans_results<-emmeans(modNZG1, ~P*CROP, at=list(N=0))
emmeans_df <- as.data.frame(emmeans_results)
merged_df <- emtrends_df %>%
  inner_join(emmeans_df, by = c("P", "CROP")) %>%
  select(P, CROP, emmean, N.trend, SE.x)
...
sorted_df <- new_df %>%
  arrange(P)
view(sorted_df)
#write.csv(sorted_df, "data/Final_Data/Nrate_esteff_grain_yr3_PC.csv")
...
NdXSIG$P <- factor(NdXSIG$P, levels = desired_order)
# finding values for the function for each yr:location  
emtrends_resultZ<-emtrends(modNZG1,~ P*CROP, var= "N", infer=TRUE)
  ...
  desired_order_P <- c("MN", "NY", "KS", "WI")
  emtrends_dfZ$PC <- paste0(emtrends_dfZ$P,emtrends_dfZ$CROP)
  emtrends_dfZ$P <- factor(emtrends_dfZ$P, levels = desired_order_P)
emmeans_resultsZ<-emmeans(modNZG1, ~P*CROP, at=list(N=0))
  emmeans_dfZ <- as.data.frame(emmeans_resultsZ)
  emmeans_dfZ$PC <- paste0(emmeans_dfZ$P,emmeans_dfZ$CROP)
mergedZ <- emtrends_dfZ %>%
  inner_join(emmeans_dfZ, by = c("PC")) %>%
  dplyr::select(PC, P.x, CROP.x, emmean, N.trend, SE.x, plvl)
  colnames(mergedZ)[2] <- "P"
  colnames(mergedZ)[3] <- "CROP"
mergedZ$P <- factor(mergedZ$P, levels = c("MN", "NY", "KS", "WI"))  # Preferred order for P
Z_df <- mergedZ %>%
  mutate(equation = sprintf("=%.2f*e(%.4fN)", exp(emmean), N.trend),  SE.x = sprintf("%.4f", SE.x),  percent_change = sprintf("%.2f%%", (exp(N.trend) - 1) * 100))%>%
  select(-emmean, -N.trend)
sortedZ <- Z_df %>%
  arrange(P)
sortedZ
sortedZ_I <- subset(sortedZ, CROP== "inter")
sortedZ_M <- subset(sortedZ, CROP== "monoI")
...
#plot
GraincomboplotNEZ <- ggplot(data=NdZSI) +
  geom_point(data = NdZSI, aes(x = N, y = G, shape = C)) +
  labs(x = "N-Rate (Kg ha-1)", y = "Grain Yield", linetype = "Cropping System", shape = "Cropping System", color = "Harvest")+
  scale_x_continuous(breaks = c(0, 40, 80, 120, 160)) +
  ylim(0, 1000) +
  facet_wrap(~P, scales = "free_y", ncol = 2) +
  scale_shape_manual(values = c(4, 25),labels = c("Intercropped","Monoculture IWG"))+
  # functions come from equation in sorted
  geom_function(data=subset(NdZSI, P== "MN" & CROP == "inter"), fun=~5.78*exp(0.0228*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "MN" & CROP == "monoI"), fun=~368.48*exp(0.0010*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "NY" & CROP == "inter"), fun=~25.39*exp(0.0002*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "NY" & CROP == "monoI"), fun=~42.64*exp(-0.0007*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "KS" & CROP == "inter"), fun=~63.89*exp(-0.0006*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "KS" & CROP == "monoI"), fun=~30.16*exp(-0.0021*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "WI" & CROP == "inter"), fun=~36.46*exp(0.0079*.x),color='darkgrey')+
  geom_function(data=subset(NdZSI, P== "WI" & CROP == "monoI"), fun=~66.34*exp(0.0002*.x),color='darkgrey')+
  geom_text(data = sortedZ_I, mapping = aes(x = 30, y = 1000, label = equation),  size=2)+ # add equation to figure
  geom_point(data = sym_I, aes(x = x_sym, y = y_sym), shape = 4, size = 1, inherit.aes = FALSE)+
  geom_text(data = sortedZ_M, mapping = aes(x = 30, y = 950, label = equation),  size=2)+ # add equation to figure
  geom_point(data = sym_M, aes(x = x_sym, y = y_sym), shape = 25, size = 1, fill = "lightgrey", stroke = 0.6, inherit.aes = FALSE)+
  geom_text(data = sortedZ, mapping = aes(x = 60, y = 999, label = plvl)) # add stars to figure
print(GraincomboplotNEZ)
# ggsave("figures/grain/GrainplotNEZ.jpg", plot = GraincomboplotNEZ, width = 8, height = 6, dpi = 300)

#figure E:C (ZS)
NdZSI$predicted <- predict(modNZG1, newdata = NdZSI, re.form = NA)
ZSGemmsPC <- emmeans(modNZG1, ~ P*CROP, type = "response") # Obtain estimated marginal means (excluding N)
emms_df <- as.data.frame(ZSGemmsPC)
cld_dataZSGPC <- cld(ZSGemmsPC, Letters=custom_letters)
cld_df <-as.data.frame(cld_dataZSGPC)
merged_df <- merge(emms_df, cld_df,by = c("P", "CROP"))
# Custom labels
custom_labels <- c("Intercrop", "Monoculture IWG")
xlabels <- c("Kansas", "Minnesota", "New York", "Wisconsin")
GZSPC<-ggplot(NdZSI, aes(x = P, y = exp(predicted), shape = CROP)) +
  geom_jitter(width = 0.2, height = 0, size = 3, alpha = 0.7) +
  labs(x = "Environment", y = "Grain Yield (kg ha-1)") +
  geom_point(data = emms_df, aes(x = P, y = response), 
             size = 4, shape = 18, color = "black")+ 
  geom_errorbar(data = emms_df, aes(x = P, ymin = lower.CL, ymax = upper.CL), 
                width = 0.2, color = "black", inherit.aes = FALSE) +
  geom_text(data = merged_df, aes(x = P, y = response.x, label = .group), 
            color = "black", vjust = 0, hjust = 2.5, size=6)+ #3
  scale_shape_manual(name = "Cropping System", labels = custom_labels, values = c(4, 25))+
#     ggtitle("Year 3 Summer \nbacktransformed for visualization")+
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        legend.title=element_text(size=14)) #change font size of legend title
print(GZSPC)
#ggsave("figures/Paper/GZSPC.jpg", plot = GZSPC, width = 10, height = 8, dpi = 300)


# mean grain yield across P and N rate (gives mean for each CROP)
NdZSI %>%
  dplyr::group_by(CROP) %>%
  dplyr:: summarise(meanG = mean(G, na.rm = TRUE), .groups = "drop")%>%
  flextable()
```
...
NG_stars <-NG_stars_tibble |> 
  select(year, term, stars) |>
  pivot_wider(names_from = term, values_from = stars)|>
  mutate(model = c("log", "log", "log"))|>
...
emtrends_resultZ<-emtrends(modNZG1,~ P, var= "N", infer=TRUE)
  emtrends_dfZ <- as.data.frame(emtrends_resultZ)
  desired_order_P <- c("MN", "NY", "KS", "WI")
  emtrends_dfZ$P <- factor(emtrends_dfZ$P, levels = desired_order_P)
emmeans_resultsZ<-emmeans(modNZG1, ~P, at=list(N=0))
emmeans_dfZ <- as.data.frame(emmeans_resultsZ)
mergedZ <- emtrends_dfZ %>%
...
GraincomboplotNE <- ggplot(data=NdXSIG) +
  ...
  geom_function(data=subset(NdYSI, P== "MN"), fun=~56.23*exp(0.0021*.x),color='blue')+
  geom_function(data=subset(NdYSI, P== "NY"), fun=~241.90*exp(0.0010*.x),color='blue')+
  geom_function(data=subset(NdYSI, P== "KS"), fun=~113.83*exp(-0.0005*.x),color='blue')+
  geom_function(data=subset(NdYSI, P== "WI"), fun=~307.25*exp(0.0065*.x),color='blue')+
  geom_function(data=subset(NdZSI, P== "MN"), fun=~46.13*exp(0.0119*.x),color='orange')+
  geom_function(data=subset(NdZSI, P== "NY"), fun=~32.90*exp(-0.0002*.x),color='orange')+
  geom_function(data=subset(NdZSI, P== "KS"), fun=~43.90*exp(-0.0014*.x),color='orange')+
  geom_function(data=subset(NdZSI, P== "WI"), fun=~49.18*exp(0.0040*.x),color='orange')
...
legend <- get_legend(GZSPC)

# Remove legends
GXSPC <- GXSPC + labs(title="a") + theme(legend.position = "none",
                       plot.title = element_text(size = 16, hjust = 0),
                       axis.title.x = element_blank(),
                       axis.title.y = element_blank())
GZSPC <- GZSPC + labs(title="b") + theme(legend.position = "none",
                       plot.title = element_text(size = 16, hjust = 0),
                       axis.title.x = element_blank(),
                       axis.title.y = element_blank())

GEC<-grid.arrange(GXSPC, GZSPC, legend, ncol = 3)
...
GEC <- plot_grid(
  ...
  legend, 
  ncol = 3, 
  rel_widths = c(1, 1, 0.3) # shrink legend width
)
GEC

test <- ggdraw(GEC) +
  draw_label("Environment", x = 0.45, y = 0.0005, vjust = 0.5, size = 18) +
  draw_label("Grain Yield (kg ha-1)", x = 0.0005, y = 0.5, angle = 90, vjust = 0.5, size = 18)+
  theme(plot.margin = margin(10, 5, 15, 20))
test

#save figures/Paper/GEC_XZ  1250x780
# 1450 x 1050
```
...
cld_dataNGPLERX1P$lower.CL <- format(cld_dataNGPLERX1P$lower.CL, digits = 2) # lower confidence interval
cld_dataNGPLERX1P$upper.CL <- format(cld_dataNGPLERX1P$upper.CL, digits = 3) # upper confidence interval 
...
mean(GLERX$LER, na.rm = TRUE)

test <- GLERX %>%
group_by(P) %>%
summarise(
avg_value = mean(LER, na.rm = TRUE)
)
flextable(data = test)
...
flextable(data = testY)
testY <- GLERY %>%
group_by(P, NRATE) %>%
summarise(
avg_value = mean(LER, na.rm = TRUE)
)%>% arrange(P, as.numeric(NRATE))
flextable(data = testY)
```
...
flextable(data = testZ)
testZ <- GLERZ %>%
group_by(NRATE) %>%
summarise(
avg_value = mean(LER, na.rm = TRUE)
)%>% arrange(as.numeric(NRATE))
flextable(data = testZ)
...
aov.tableX <- anova(modGPLERX1)
tablePLERX = flextable(data = aov.tableX %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GPLERX 

aov.tableY <- anova(modGPLERY1)
tablePLERY = flextable(data = aov.tableY %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GPLERY

aov.tableZ <- anova(modGPLERZ1)
tablePLERZ = flextable(data = aov.tableZ %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GPLERZ

my_modelsGPLER <- c(modGPLERX1, modGPLERY1, modGPLERZ1)
anovasGPLER <- purrr::map(my_modelsGPLER, ~anova(.))
names(anovasGPLER) <- c("Year 1", "Year 2",  "Year 3")
...
# Apply the function to ANOVA results
GPLER_stars_tibble <- map(anovasGPLER, as_tibble, rownames="term") %>%
  bind_rows(.id = "model") %>%
  mutate(stars = map_chr(`Pr(>F)`, format_p_value))

GPLER_stars_tibble <- GPLER_stars_tibble %>%
  mutate(term = ifelse(term == "P", "E", term)) %>%
  mutate(term = ifelse(term == "N:P", "N:E", term))%>%
...
NG_stars <-GPLER_stars_tibble |> 
  select(model, term, stars) |>
  pivot_wider(names_from = term, values_from = stars)

gt_NG <- gt(NG_stars)

# gt_NG |> gtsave("figures/grain/gt_GPLER.png", expand = 10)
```

# PLER CI Tables

```{r}
# Table of pairwise comparions of GLPLER by site with confidence intervals.

cld_dataNGPLERX1P <- arrange(cld_dataNGPLERX1P, Site)
cld_dataNGPLERYP <- arrange(cld_dataNGPLERYP, Site)
cld_dataNGPLERZP <- arrange(cld_dataNGPLERZP, Site)

GLPLER_table_merge <- cbind(cld_dataNGPLERX1P, cld_dataNGPLERYP, cld_dataNGPLERZP)
colnames(GLPLER_table_merge) <- make.unique(colnames(GLPLER_table_merge))
GLPLER_table_merge2 <- GLPLER_table_merge[,c('Site', 'GPLER', 'l', 'h','GPLER.1', 'l.1', 'h.1','GPLER.2', 'l.2', 'h.2')]

GLPLER_table_merge2 <- as.data.frame(GLPLER_table_merge2)
colnames(GLPLER_table_merge2) <- c( "Site", "GPLER","CI 25","CI 95", "GPLER","CI 25","CI 95", "GPLER","CI 25","CI 95")
...
# save as figures/grain/GPLER_CI_table
```
...
mean(NGRY_long$LER, na.rm = TRUE)

testX <- NGRY_long %>%
group_by(P) %>%
summarise(
avg_value = mean(GRelativeYield, na.rm = TRUE)
)
flextable(data = testX)
testX <- NGRY_long %>%
group_by(N) %>%
summarise(
avg_value = mean(GRelativeYield, na.rm = TRUE)
)%>% arrange(as.numeric(N))
flextable(data = testX)
...
#Pairwise P
N_levels <- c(0, 40, 80, 120, 160)
emmsP <- emmeans(modGRYY1, specs = ~ P, type = "response")
cld_dataGRYYP <- cld(emmsP, Letters=custom_letters) # Create compact letter display
cld_dataGRYYP  $response <- format(cld_dataGRYYP  $response, digits = 3)
cld_dataGRYYP $lower.CL <- format(cld_dataGRYYP $lower.CL, digits = 3)
cld_dataGRYYP $upper.CL <- format(cld_dataGRYYP $upper.CL, digits = 3)
rownames(cld_dataGRYYP  ) <- NULL
cld_dataGRYYP  <- cld_dataGRYYP   %>%
  select( P, response, lower.CL, upper.CL, .group)
colnames(cld_dataGRYYP  ) <- c( "Site", "RY","l","h", "Group")
cld_dataGRYYP    |> 
  data.frame() |> 
  kbl(align = "c") |> 
  kable_paper(full_width = F) |> 
...
mean(NGRY_longY$GRelativeYield, na.rm = TRUE)
...
testY <- NGRY_longY %>%
group_by(P, N) %>%
summarise(avg_value = mean(GRelativeYield, na.rm = TRUE)
)%>% arrange(P, as.numeric(N))
flextable(data = testY)
```
...
#Pairwise comparisons
#Pairwise P
emmsP <- emmeans(modGRYZ1, specs = ~ P, type = "response")
cld_dataGRYZP <- cld(emmsP, Letters=custom_letters) # Create compact letter display
cld_dataGRYZP$response <- format(cld_dataGRYZP$response, digits = 3)
cld_dataGRYZP$lower.CL <- format(cld_dataGRYZP$lower.CL, digits = 3)
cld_dataGRYZP$upper.CL <- format(cld_dataGRYZP$upper.CL, digits = 3)
rownames(cld_dataGRYZP) <- NULL
cld_dataGRYZP  <- cld_dataGRYZP%>%
  select(P, response, lower.CL, upper.CL, .group)
colnames(cld_dataGRYZP) <- c( "Site", "RY","l","h", "Group")
cld_dataGRYZP    |> 
  data.frame() |> 
  kbl(align = "c") |> 
  kable_paper(full_width = F) |> 
...
mean(NGRY_longZ$GRelativeYield, na.rm = TRUE)
...
```{r}
aov.tableRYX <- anova(modGRYX1)
tableRYX = flextable(data = aov.tableRYX %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GRYX 

aov.tableRYY <- anova(modGRYY1)
tableRYY = flextable(data = aov.tableRYY %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GRYY

aov.tableRYZ <- anova(modGRYZ1)
tableRYZ = flextable(data = aov.tableRYZ %>%
                    #filter(`Pr(>F)` < 0.05) %>%
                    mutate(`Pr(>F)` = formatC(`Pr(>F)`, digits = 3)) %>% 
                    select(`Pr(>F)`) %>%
                    rownames_to_column(" ")) 
# save figures/grain/gt_GRYZ

my_modelsGRY <- c(modGRYX1, modGRYY1, modGRYZ1)
anovasGRY <- purrr::map(my_modelsGRY, ~anova(.))
names(anovasGRY) <- c("Year 1", "Year 2",  "Year 3")
...
GRY_stars_tibble <- GRY_stars_tibble %>%
  mutate(term = ifelse(term == "P", "E", term)) %>%
  mutate(term = ifelse(term == "N:P", "N:E", term))%>%
  mutate(term = ifelse(term == "P:C", "E:C", term))%>%
  mutate(term = ifelse(term == "N:P:C", "N:E:C", term))
...
NG_stars <-GRY_stars_tibble |> 
  select(model, term, stars) |>
  pivot_wider(names_from = term, values_from = stars)

gt_NG <- gt(NG_stars)

# gt_NG |> gtsave("figures/grain/gt_GRY.png", expand = 10)
```
...
cld_dataGRYXP <- arrange(cld_dataGRYXP, Site)
cld_dataGRYYP <- arrange(cld_dataGRYYP, Site)
cld_dataGRYZP <- arrange(cld_dataGRYZP, Site)

GRY_table_merge <- cbind(cld_dataGRYXP, cld_dataGRYYP, cld_dataGRYZP)
colnames(GRY_table_merge) <- make.unique(colnames(GRY_table_merge))
GRY_table_merge2 <- GRY_table_merge[,c('Site', 'GRY', 'l', 'h','RY', 'l.1', 'h.1','RY.1', 'l.2', 'h.2')]

GRY_table_merge2 <- as.data.frame(GRY_table_merge2)
colnames(GRY_table_merge2) <- c( "Site", "GRY","CI 25","CI 95", "GRY","CI 25","CI 95", "GRY","CI 25","CI 95")
...
# save as figures/grain/GRY_CI_table
```
