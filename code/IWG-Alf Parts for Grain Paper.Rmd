...
GXSPC<-ggplot(NdXSIG, aes(x = P, y = exp(predicted), shape = CROP)) +
  ...
  geom_text(data = merged_df, aes(x = P, y = response.x, label = .group), 
            color = "black", vjust = 0, hjust = 2.15)+
...
GcomboplotNEXS <- ggplot(data=NdXSI) +
  ...
  geom_function(data=subset(NdXSIG, P== "NY"), fun=~408.24*exp(0.0047*.x),color='darkgrey')+
  geom_function(data=subset(NdXSIG, P== "WI"), fun=~736.77*exp(-0.0001*.x),color='darkgrey')+
  geom_text(data = sortedX, mapping = aes(x = 25, y = 1800, label = equation),  size=4)+ # add equation to figure
  geom_text(data = sortedX, mapping = aes(x = 58, y = 1800, label = plvl), size=4)+ # add stars to figure
# add stars to figure
...
# Remove legends
GXSPC <- GXSPC + theme(legend.position = "none",
                       axis.title.x = element_blank(),
                       axis.title.y = element_blank())
GZSPC <- GZSPC + theme(legend.position = "none",
                       axis.title.x = element_blank(),
                       axis.title.y = element_blank())
...
test <- ggdraw(GEC) +
  draw_label("Environment", x = 0.45, y = 0.0005, vjust = 0.5, size = 16) +
  draw_label("Grain Yield (kg ha-1)", x = 0.0005, y = 0.5, angle = 90, vjust = 0.5, size = 16)+
