...
modNZFT<-lm(PlotTotForageKgHa~N*P*CROP, data=NdZF_2) #modified until more locations 
anova(modNYFT)
summary(modNZFT)
#Check Assumptions
...
modNZFT1<-lm(log(PlotTotForageKgHa)~N*P*CROP, data=NdZF_2) #modified until more locations 
...
modN<-lmer(PlotTotForageKgHa~N*P*CROP + (1|YearCode) + (1|Rep), data=NdS)
...
#rec
modNXSA1N<-lmer(log(PlotAlfDryKgHa)~N+P*CROP + (1|Rep), data=NdXSA)
...
#Pairwise P
emmsP <- emmeans(modNXSA1, ~ P, type = "response") # Obtain estimated marginal means
cld_dataNXSAP <- cld(emmsP, Letters=custom_letters) # Create compact letter display
#Pairwise C
emmsC <- emmeans(modNXSA1, ~ CROP, type = "response") # Obtain estimated marginal means 
cld_dataNXSAC <- cld(emmsC, Letters=custom_letters) # Create compact letter display
#Pairwise N
N_levels <- c(0, 40, 80, 120, 160)
emmsN <- emmeans(modNXSA1, specs = ~ N, at = list(N = N_levels), type = "response")
...
cld_dataNXSAN  |> 
  ...
  column_spec(1, bold = T)
#Pairwise P:C
emmsPC <- emmeans(modNXSA1, ~ P*CROP, type= "response") # Obtain estimated marginal means
...
#This gives us the slope for each interactions as well as a P value indicating significance level for that slope
emtrends(modNXSA1,~ P*CROP, var= "N", infer=TRUE)
emtrends(modNXSA1,~ P, var= "N", infer=TRUE)
emtrends(modNXSA1,~ CROP, var= "N", infer=TRUE)
emtrends(modNXSA1,~N, var= "N", infer=TRUE)
summary(modNXSA1)
emtrends(modNXSA1,~ P, var= "N", infer=TRUE)
