...
# remove empty values
NTFIRY0 <- NTFIRY0[!is.na(NTFIRY0$TFIRelativeYield0), ]
NTFIRY40 <- NTFIRY40[!is.na(NTFIRY40$TFIRelativeYield40), ]
NTFIRY80 <- NTFIRY80[!is.na(NTFIRY80$TFIRelativeYield80), ]
NTFIRY120 <- NTFIRY120[!is.na(NTFIRY120$TFIRelativeYield120), ]
NTFIRY160 <- NTFIRY160[!is.na(NTFIRY160$TFIRelativeYield160), ]
...
NTFIRYZ<-NTFIRY_long[NTFIRY_long$YearCode== "Z",]
...
NTFIRYZS<-NTFIRYZ[NTFIRYZ$Harvest== "Summer",]
NTFIRYZF<-NTFIRYZ[NTFIRYZ$Harvest== "Fall",]
class(NTFIRYXS$N)
#models 
#year 1 summer TFI RY
{
modTFIRYXS<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYXS)
anova(modTFIRYXS)
#check assumptions 
...
plot(modTFIRYXS)
#Logtransform
modTFIRYXS1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYXS)
anova(modTFIRYXS1)
#check LOG assumptions 
...
#year 1 Fall TFI RY
{
#modTFIRYXF<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYXF)
#boundary (singular) fit: see help('isSingular')
#anova(modTFIRYXF)
#check assumptions 
...
#plot(modTFIRYXF)
#Logtransform
modTFIRYXF1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYXF)
anova(modTFIRYXF1)
#check LOG assumptions 
...
{
modTFIRYYS<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYYS)
anova(modTFIRYYS)
#check assumptions 
...
plot(modTFIRYYS)
#Logtransform
modTFIRYYS1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYYS)
anova(modTFIRYYS1)
#check LOG assumptions 
...
#year 2 Fall TFI RY
{
modTFIRYYF<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYYF)
anova(modTFIRYYF)
#check assumptions 
...
plot(modTFIRYYF)
#Logtransform
modTFIRYYF1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYYF)
anova(modTFIRYYF1)
#check LOG assumptions 
...
#year 3 summer TFI RY
{
modTFIRYZS<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYZS)
anova(modTFIRYZS)
#check assumptions 
TFIRYZS_resid<-resid(modTFIRYZS)
qqnorm(TFIRYZS_resid)
qqline(TFIRYZS_resid)
plot(modTFIRYZS)
#Logtransform
modTFIRYZS1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYZS)
anova(modTFIRYZS1)
#check LOG assumptions 
TFIRYZS_resid1<-resid(modTFIRYZS1)
qqnorm(TFIRYZS_resid1)
qqline(TFIRYZS_resid1)
plot(modTFIRYZS1)
}
#Pairwise N:P
N_levels <- c(0, 40, 80, 120, 160)
emmsNP <- emmeans(modTFIRYZS1, specs = ~ N*P, at = list(N = N_levels), type = "response")
cld_dataNTFIRYZSNP <- cld(emmsNP, Letters=custom_letters) # Create compact letter display
cld_dataNTFIRYZSNP  $response <- format(cld_dataNTFIRYZSNP  $response, digits = 3)
cld_dataNTFIRYZSNP $lower.CL <- format(cld_dataNTFIRYZSNP $lower.CL, digits = 3)
cld_dataNTFIRYZSNP $upper.CL <- format(cld_dataNTFIRYZSNP $upper.CL, digits = 3)
rownames(cld_dataNTFIRYZSNP  ) <- NULL
cld_dataNTFIRYZSNP  <- cld_dataNTFIRYZSNP   %>%
  select( N, P, response, lower.CL, upper.CL, .group)
colnames(cld_dataNTFIRYZSNP  ) <- c( "N", "Site", "TIFRY","l","h", "Group")
cld_dataNTFIRYZSNP    |> 
  data.frame() |> 
  kbl(align = "c") |> 
  kable_paper(full_width = F) |> 
...
#year 2 Fall TFI RY
{
modTFIRYZF<-lmer(TFIRelativeYield~N*P + (1|P:Rep), data=NTFIRYZF)
anova(modTFIRYZF)
#check assumptions 
TFIRYZF_resid<-resid(modTFIRYZF)
qqnorm(TFIRYZF_resid)
qqline(TFIRYZF_resid)
plot(modTFIRYZF)
#Logtransform
modTFIRYZF1<-lmer(log(TFIRelativeYield)~N*P + (1|P:Rep), data=NTFIRYZF)
anova(modTFIRYZF1)
#check LOG assumptions 
TFIRYZF_resid1<-resid(modTFIRYZF1)
qqnorm(TFIRYZF_resid1)
qqline(TFIRYZF_resid1)
plot(modTFIRYZF1)
}
#Pairwise P
emmsP <- emmeans(modTFIRYZF1, ~ P, type = "response") # Obtain estimated marginal means
cld_dataTFIRYZFP <- cld(emmsP, Letters=custom_letters) # Create compact letter display
cld_dataTFIRYZFP  $response <- format(cld_dataTFIRYZFP  $response, digits = 3)
cld_dataTFIRYZFP $lower.CL <- format(cld_dataTFIRYZFP $lower.CL, digits = 3)
cld_dataTFIRYZFP $upper.CL <- format(cld_dataTFIRYZFP $upper.CL, digits = 3)
rownames(cld_dataTFIRYZFP  ) <- NULL
cld_dataTFIRYZFP  <- cld_dataTFIRYZFP   %>%
  select( P, response, lower.CL, upper.CL, .group)
colnames(cld_dataTFIRYZFP  ) <- c( "Site", "TIFRY","l","h", "Group")
cld_dataTFIRYZFP    |> 
  data.frame() |> 
  kbl(align = "c") |> 
  kable_paper(full_width = F) |> 
...
NTFIRYZS$N <- factor(NTFIRYZS$N, levels = factor_levels)
NTFIRYXF$N <- factor(NTFIRYXF$N, levels = factor_levels)
NTFIRYYF$N <- factor(NTFIRYYF$N, levels = factor_levels)
NTFIRYZF$N <- factor(NTFIRYZF$N, levels = factor_levels)
#year 1 summer 
...
#year 3 summer 
ggplot(NTFIRYZS, aes(x = N, y = TFIRelativeYield)) +
  geom_boxplot() +
  facet_wrap(~P, scales = "free_y",ncol=2) +
  labs(x = "N Rate", y = "Relative Yield") +
  theme_minimal()+
   scale_x_discrete(breaks = c(0, 40, 80, 120, 160))+
    geom_abline(intercept = 0.5, slope = 0, color = "red", linetype = "dashed")+
    ggtitle("Year 3 Summer Total Forage Yield Relative to IWG monoculture")+
  ylim(0.5, NA)
#year 2 Fall 
ggplot(NTFIRYZF, aes(x = N, y = TFIRelativeYield)) +
  geom_boxplot() +
  facet_wrap(~P, scales = "free_y",ncol=2) +
  labs(x = "N Rate", y = "Relative Yield") +
  theme_minimal()+
   scale_x_discrete(breaks = c(0, 40, 80, 120, 160))+
    geom_abline(intercept = 0.5, slope = 0, color = "red", linetype = "dashed")+
    ggtitle("Year 3 Fall Total Forage Yield Relative to IWG monoculture")+
  ylim(0.5, NA)
```
...
NTFIRYZS$Year<-"Year 3 Summer"
NTFIRYZF$Year<-"Year 3 Fall"

combined_dataTFIRY <- rbind(NTFIRYXS, NTFIRYXF,NTFIRYYS,NTFIRYYF, NTFIRYZS, NTFIRYZF)

colors2 <- c("Year 1 Summer" = "red", 
            "Year 1 Fall" = "green", 
            "Year 2 Summer" = "blue", 
            "Year 2 Fall" = "purple", 
            "Year 3 Summer" = "orange",
            "Year 3 Fall" = "yellow")
legend_order3 <- c("Year 1 Summer", "Year 1 Fall", 
                  "Year 2 Summer", "Year 2 Fall", "Year 3 Summer", "Year 3 Fall")
